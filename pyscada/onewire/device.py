        ...
        if self.device.onewiredevice.adapter_type == "rpi_gpio4":
            ...
            for item in self.variables:
                ...
                if item.onewirevariable.address.lower() in w1_slaves:
                    f = open(
                        "/sys/bus/w1/devices/"
                        + str("28-" + item.onewirevariable.address)
                        + "/w1_slave"
                    )
                    filecontent = f.read()
                    f.close()
        ...
        # OWServer
            ...
            try:
                owproxy = pyownet.protocol.proxy(
                    host=hostname, port=port, flags=pyownet.protocol.FLG_UNCACHED
                )
            except:
                return None
            w1_slaves = []
                ...
                if item.onewirevariable.sensor_type in ["DS18B20"]:
                    if "/28.%s/" % item.onewirevariable.address.lower() in w1_slaves:
                        ...
                        try:
                            value = float(
                                owproxy.read(
                                    "/28.%s/temperature" % item.onewirevariable.address
                                )
                            )
                        except:
                            value = None
