...
class OneWireVariable(models.Model):
    ...
    address = models.CharField(
        default="", max_length=400, help_text="64bit Sensor Address"
    )
    sensor_type_choices = (("DS18B20", "DS18B20 Temperature Sensor"),)
    sensor_type = models.CharField(
        default=sensor_type_choices[0][0], max_length=10, choices=sensor_type_choices
    )
...
class OneWireDevice(models.Model):
    ...
    adapter_type_choices = (
        ("owserver", "OWFS owserver"),
        ("rpi_gpio4", "RPi GPIO 4"),
    )
    adapter_type = models.CharField(
        default=adapter_type_choices[0][0], max_length=400, choices=adapter_type_choices
    )
